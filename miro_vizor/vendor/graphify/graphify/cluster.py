...
def cluster(G: nx.Graph) -> dict[int, list[str]]:
    ...
    if connected.number_of_nodes() > 0:
        communities_iter = nx_comm.louvain_communities(connected, seed=42)
        for cid, members in enumerate(communities_iter):
            for node in members:
...
def _split_community(G: nx.Graph, nodes: list[str]) -> list[list[str]]:
    ...
    try:
        import networkx.algorithms.community as nx_comm
        sub_communities_iter = nx_comm.louvain_communities(subgraph, seed=42)
        sub_communities: dict[int, list[str]] = {}
        for cid, members in enumerate(sub_communities_iter):
            for node in members:
                sub_communities.setdefault(cid, []).append(node)
        if len(sub_communities) <= 1:
            # Louvain couldn't split it - return as-is
            return [sorted(nodes)]
